...
export default function AdminDashboard() {
  ...
  const [mhsForm, setMhsForm] = useState({ nim: "", nama_mahasiswa: "" });
  const [mhsMsg, setMhsMsg] = useState("");
  ...
  const addMK = async () => {
    ...
    if (res.ok) { setMsg("Berhasil!"); setMkForm({ kode_mk: "", nama_mk: "", sks: "", nidn: "", hari: "", waktu: "", ruangan: "" }); setShowAddMK(false); fetchData(); }
  ...
  const addMahasiswa = async () => {
    ...
    // Validasi Frontend
    if (!/^\d+$/.test(mhsForm.nim)) {
      setMhsMsg("NIM hanya boleh berisi angka.");
      return;
    }
    if (mhsForm.nim.length < 8) {
      setMhsMsg("NIM minimal harus terdiri dari 8 karakter.");
      return;
    }
    if (!mhsForm.nama_mahasiswa.trim()) {
      setMhsMsg("Nama Mahasiswa tidak boleh kosong.");
      return;
    }

    const res = await fetch("/api/admin/mahasiswa", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(mhsForm) });
    const d = await res.json();
    if (res.ok) { 
      setMhsMsg("Berhasil ditambahkan!"); 
      setMhsForm({ nim: "", nama_mahasiswa: "" }); 
      setShowAddMhs(false); 
      fetchData(); 
    }
    else setMhsMsg(d.message);
  };

  const deleteMahasiswa = async (nim: string) => {
    if (!confirm("Hapus mahasiswa ini? Semua data presensi miliknya juga akan hilang secara kaskade (cascade) dari database.")) return;
  ...
  if (!data) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center text-amber-500">
      ...
      {msg && (
        <button onClick={() => window.location.href = "/login"} className="mt-4 px-4 py-2 bg-stone-800 text-white rounded-xl">
          Kembali ke Login
        </button>
      )}
    </div>
  ...
  return (
    <div className="h-screen bg-background text-foreground flex flex-col md:flex-row font-sans selection:bg-amber-500/30 overflow-hidden relative transition-colors duration-300">
      ...
      {/* Sidebar */}
      <aside className="w-full md:w-[300px] border-b md:border-b-0 md:border-r border-foreground/10 p-4 md:p-6 flex flex-row md:flex-col h-auto md:h-full flex-shrink-0 bg-background/80 backdrop-blur-xl z-50">
        <div className="mb-0 md:mb-12 flex items-center gap-3 shrink-0 pr-4 md:pr-0">
          ...
          <div>
            ...
            <p className="text-[10px] text-stone-500 dark:text-stone-400 dark:text-stone-500 uppercase tracking-widest font-bold">STIKOM 22 Januari</p>
          </div>
        </div>
          ...
          {activeTab === "Dashboard" && (<>
              ...
              {[
                ...
                <TiltCard key={i} className={`border dark:border-white/5 border-stone-200 dark:border-white/5 ${s.glow}`}>
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`w-10 h-10 rounded-xl dark:bg-white/5 bg-stone-100 dark:bg-white/5 flex items-center justify-center ${s.color}`}><s.icon size={20} /></div>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <TiltCard className="border dark:border-white/5 border-stone-200 dark:border-white/5">
                ...
                <div className="flex justify-center gap-6 mt-2">
                  <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-green-500" /><span className="text-xs text-stone-600 dark:text-stone-300">Hadir ({pieData[0].value})</span></div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-500" /><span className="text-xs text-stone-600 dark:text-stone-300">Luar ({pieData[1].value})</span></div>
                </div>
              </TiltCard>
            </div>
            ...
            <TiltCard className="border dark:border-white/5 border-stone-200 dark:border-white/5">
              ...
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                    {(data.presensiTerbaru || []).slice(0, 8).map((p: any, i: number) => (
                      <tr key={i} className="border-b dark:border-white/5 border-stone-200 dark:border-white/5 hover:dark:bg-white/5 bg-stone-100 dark:bg-white/5 transition-colors">
                        ...
                        <td className="py-3 text-stone-600 dark:text-stone-300">{p.dosen?.nama_dosen || p.nidn}</td>
                ...
                {(!data.presensiTerbaru || data.presensiTerbaru.length === 0) && <p className="text-center py-8 text-stone-500 dark:text-stone-400">Belum ada data presensi.</p>}
              </div>
            </TiltCard>
          </>)}
          ...
          {activeTab === "Data Mahasiswa" && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <button onClick={() => setShowAddMhs(true)} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 text-black font-bold flex items-center gap-2 hover:scale-105 transition-transform shadow-[0_0_20px_rgba(245,158,11,0.3)]">
              ...
              {showAddMhs && (
                <TiltCard className="border border-amber-500/30 bg-gradient-to-br from-amber-500/5 to-transparent">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold text-stone-800 dark:text-white">Tambah Mahasiswa Baru</h3>
                    <button onClick={() => setShowAddMhs(false)} className="text-stone-600 dark:text-stone-400 hover:text-stone-900 dark:hover:text-white"><X size={20} /></button>
                  </div>
                  {mhsMsg && <p className="text-sm font-bold text-amber-600 dark:text-amber-400 mb-3">{mhsMsg}</p>}
                  <div className="flex flex-col sm:flex-row gap-4">
                    <input 
                      value={mhsForm.nim} 
                      onChange={e => setMhsForm({ ...mhsForm, nim: e.target.value })} 
                      placeholder="NIM Mahasiswa" 
                      className="flex-1 bg-white dark:bg-black/50 border border-stone-300 dark:border-white/10 rounded-xl p-3 text-stone-900 dark:text-white focus:border-amber-500 focus:outline-none" 
                    />
                    <input 
                      value={mhsForm.nama_mahasiswa} 
                      onChange={e => setMhsForm({ ...mhsForm, nama_mahasiswa: e.target.value })} 
                      placeholder="Nama Lengkap Mahasiswa" 
                      className="flex-[2] bg-white dark:bg-black/50 border border-stone-300 dark:border-white/10 rounded-xl p-3 text-stone-900 dark:text-white focus:border-amber-500 focus:outline-none" 
                    />
                    <button onClick={addMahasiswa} className="px-6 py-3 rounded-xl bg-amber-500 text-black font-bold hover:bg-amber-400 transition-colors">
                      Simpan
                    </button>
                  </div>
                  <p className="text-xs text-stone-500 dark:text-stone-500 mt-3">* Password akun otomatis disetel ke: <strong>stikom123</strong></p>
                </TiltCard>
              )}
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="text-left text-stone-600 dark:text-stone-400 border-b border-stone-200 dark:border-white/5">
                      <th className="pb-3 font-medium">NIM</th><th className="pb-3 font-medium">Nama Lengkap</th><th className="pb-3 font-medium">Total Presensi</th><th className="pb-3 font-medium">Aksi</th>
                    </tr></thead>
                    <tbody>
                      {(data.mahasiswa || []).map((m: any, i: number) => (
                        <tr key={i} className="border-b border-stone-200 dark:border-white/5 hover:bg-stone-50 dark:hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-4">
                            <span className="px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-500/10 text-amber-800 dark:text-amber-400 text-xs font-bold border border-amber-200 dark:border-transparent">
                              {m._count.presensi} kali hadir
                            </span>
                          </td>
                          <td className="py-4">
                            <button onClick={() => deleteMahasiswa(m.nim)} className="text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-500/10 p-2 rounded-lg transition-colors"><Trash2 size={16} /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {(data.dosen || []).map((d: any, i: number) => (
                        <tr key={i} className="border-b dark:border-white/5 border-stone-200 dark:border-white/5 hover:dark:bg-white/5 bg-stone-100 dark:bg-white/5 transition-colors">
                          <td className="py-4 font-mono text-orange-600 dark:text-orange-400">{d.nidn}</td>
                          <td className="py-4 font-medium">{d.nama_dosen}</td>
                          <td className="py-4"><span className="px-3 py-1 rounded-full bg-orange-500/10 text-orange-600 dark:text-orange-400 text-xs font-bold">{d._count.presensi} mahasiswa</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
              ...
              {showAddMK && (
                <TiltCard className="border border-amber-500/30 bg-gradient-to-br from-amber-500/5 to-transparent">
                  ...
                  {msg && <p className="text-sm text-amber-600 dark:text-amber-400 mb-3">{msg}</p>}
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <input value={mkForm.kode_mk} onChange={e => setMkForm({ ...mkForm, kode_mk: e.target.value })} placeholder="Kode MK (cth: IF101)" className="dark:bg-stone-100 dark:bg-white/50 bg-stone-100 dark:bg-white/5 border dark:border-white/10 border-stone-200 dark:border-white/10 rounded-xl p-3 text-foreground focus:border-amber-500 focus:outline-none" />
                    <input value={mkForm.nama_mk} onChange={e => setMkForm({ ...mkForm, nama_mk: e.target.value })} placeholder="Nama Mata Kuliah" className="dark:bg-stone-100 dark:bg-white/50 bg-stone-100 dark:bg-white/5 border dark:border-white/10 border-stone-200 dark:border-white/10 rounded-xl p-3 text-foreground focus:border-amber-500 focus:outline-none" />
                    <input value={mkForm.sks} onChange={e => setMkForm({ ...mkForm, sks: e.target.value })} placeholder="SKS" type="number" className="dark:bg-stone-100 dark:bg-white/50 bg-stone-100 dark:bg-white/5 border dark:border-white/10 border-stone-200 dark:border-white/10 rounded-xl p-3 text-foreground focus:border-amber-500 focus:outline-none" />
                    
                    <select value={mkForm.nidn} onChange={e => setMkForm({ ...mkForm, nidn: e.target.value })} className="dark:bg-stone-100 dark:bg-white/50 bg-stone-100 dark:bg-white/5 border dark:border-white/10 border-stone-200 dark:border-white/10 rounded-xl p-3 text-stone-600 dark:text-stone-300 focus:border-amber-500 focus:outline-none">
                    ...
                    <select value={mkForm.hari} onChange={e => setMkForm({ ...mkForm, hari: e.target.value })} className="dark:bg-stone-100 dark:bg-white/50 bg-stone-100 dark:bg-white/5 border dark:border-white/10 border-stone-200 dark:border-white/10 rounded-xl p-3 text-stone-600 dark:text-stone-300 focus:border-amber-500 focus:outline-none">
                    ...
                    <input value={mkForm.waktu} onChange={e => setMkForm({ ...mkForm, waktu: e.target.value })} placeholder="Waktu (cth: 10:00 - 12:30)" className="dark:bg-stone-100 dark:bg-white/50 bg-stone-100 dark:bg-white/5 border dark:border-white/10 border-stone-200 dark:border-white/10 rounded-xl p-3 text-foreground focus:border-amber-500 focus:outline-none" />
                    <input value={mkForm.ruangan} onChange={e => setMkForm({ ...mkForm, ruangan: e.target.value })} placeholder="Ruangan (cth: Lab Komputer 1)" className="dark:bg-stone-100 dark:bg-white/50 bg-stone-100 dark:bg-white/5 border dark:border-white/10 border-stone-200 dark:border-white/10 rounded-xl p-3 text-foreground focus:border-amber-500 focus:outline-none" />
                  </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {(data.mataKuliah || []).map((mk: any, i: number) => (
                        <tr key={i} className="border-b dark:border-white/5 border-stone-200 dark:border-white/5 hover:dark:bg-white/5 bg-stone-100 dark:bg-white/5 transition-colors">
                          <td className="py-4 font-mono text-blue-600 dark:text-blue-400">{mk.kode_mk}</td>
