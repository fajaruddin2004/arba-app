...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans overflow-x-hidden selection:bg-amber-500/30 relative transition-colors duration-300">
      ...
      >
        ...
        <div className="absolute top-[50%] left-[40%] w-[40vw] h-[40vw] rounded-full bg-[#e6ccb2]/15 dark:bg-yellow-600/5 blur-[100px] dark:mix-blend-screen animate-blob animation-delay-4000"></div>
      ...
      <div className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
        <nav className={`transition-all duration-500 pointer-events-auto flex items-center justify-between px-6 py-3 rounded-full ${isScrolled ? 'glass-pill w-full max-w-5xl' : 'w-full max-w-7xl'}`}>
          ...
          <div className="hidden md:flex items-center gap-8 bg-black/5 dark:bg-black/20 px-8 py-2 rounded-full border border-black/5 dark:border-white/5">
            <a href="#" className="text-sm font-medium text-foreground/70 hover:text-amber-500 transition-colors">Beranda</a>
            <a href="#info-kampus" className="text-sm font-medium text-foreground/70 hover:text-amber-500 transition-colors">Profil Kampus</a>
            <a href="#fitur-utama" className="text-sm font-medium text-foreground/70 hover:text-amber-500 transition-colors">Fitur Inovasi</a>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <a
              href="/login"
              className="px-6 py-2.5 rounded-full bg-foreground text-background font-bold text-sm hover:bg-amber-400 hover:text-black shadow-[0_0_20px_rgba(255,255,255,0.2)] transform transition-all hover:scale-105 flex items-center gap-2 group"
            >
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-10 items-center w-full">
          ...
          >
            ...
            <p className="text-lg text-foreground/70 leading-relaxed max-w-md font-light">
              Meninggalkan cara konvensional. Menghadirkan ekosistem akademik mutakhir dengan validasi <b className="text-foreground">Geolokasi</b> dan <b className="text-foreground">Evaluasi Dosen</b> terpusat di STIKOM 22 Januari Kendari.
            </p>
      ...
      <section id="info-kampus" className="py-16 md:py-24 relative z-10 dark:bg-[#080503] bg-white border-t dark:border-stone-800/50 border-black/5 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col items-center text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black text-foreground mb-6">
              Mengenal <span className="gradient-text">STIKOM 22 Januari.</span>
            </h2>
            <p className="text-foreground/70 max-w-2xl text-lg">Pusat pengembangan ilmu pengetahuan bidang Sistem Informasi dan Teknik Komputer di jantung Kota Kendari.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 auto-rows-[minmax(180px,auto)]">
            ...
            <div className="md:col-span-2 lg:col-span-6 glass-card rounded-[2.5rem] p-8 flex flex-col justify-between">
              <div className="flex gap-2 mb-6">
                <button
                  ...
                  className={`px-5 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'visi' ? 'bg-amber-500 text-black shadow-[0_0_15px_rgba(217,119,6,0.4)]' : 'dark:bg-stone-900 bg-black/5 text-foreground/70 hover:text-foreground'}`}
                >Visi 2040</button>
                <button
                  onClick={() => setActiveTab('sejarah')}
                  className={`px-5 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'sejarah' ? 'bg-amber-500 text-black shadow-[0_0_15px_rgba(217,119,6,0.4)]' : 'dark:bg-stone-900 bg-black/5 text-foreground/70 hover:text-foreground'}`}
                >Sejarah</button>
              </div>
            ...
            <TiltCard intensity={5} className="md:col-span-1 lg:col-span-3">
              <div className="h-full glass-card rounded-[2rem] p-8 flex flex-col justify-center border-t-2 border-blue-500/30 group hover:border-blue-500/60 transition-colors">
                <Monitor className="text-blue-500 dark:text-blue-400 mb-4" size={32} />
                <h4 className="text-xl font-bold text-foreground mb-2">Sistem Informasi</h4>
                <p className="text-foreground/70 text-xs leading-relaxed">Fokus pengembangan rekayasa perangkat lunak dan manajemen sistem.</p>
              </div>
            </TiltCard>
            ...
            <TiltCard intensity={5} className="md:col-span-1 lg:col-span-3">
              <div className="h-full glass-card rounded-[2rem] p-8 flex flex-col justify-center border-t-2 border-amber-500/30 group hover:border-amber-500/60 transition-colors">
                <Cpu className="text-amber-600 dark:text-amber-500 mb-4" size={32} />
                <h4 className="text-xl font-bold text-foreground mb-2">Teknik Komputer</h4>
                <p className="text-foreground/70 text-xs leading-relaxed">Fokus pada arsitektur perangkat keras, jaringan, dan IoT masa depan.</p>
              </div>
            </TiltCard>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16 mb-32">
            ...
            <div className="w-full lg:w-1/2 space-y-6">
              ...
              <h3 className="text-3xl lg:text-4xl font-black text-foreground">Presensi Anti-Manipulasi berbasis <span className="text-amber-500">Geofencing GPS.</span></h3>
              ...
              <ul className="space-y-4 pt-4">
                <li className="flex items-start gap-3">
                  ...
                  <p className="text-foreground/80"><b>Akurasi Spasial:</b> Presensi hanya berhasil jika mahasiswa fisik berada dalam radius STIKOM 22 Januari.</p>
                </li>
                <li className="flex items-start gap-3">
                  <CheckCircle2 className="text-amber-500 mt-1 shrink-0" size={20} />
                  <p className="text-foreground/80"><b>Real-time Dashboard:</b> Nama mahasiswa yang berhasil absen langsung muncul di layar dosen saat itu juga.</p>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row-reverse items-center gap-16">
            ...
            <div className="w-full lg:w-1/2 space-y-6">
              ...
              <h3 className="text-3xl lg:text-4xl font-black text-foreground">Evaluasi Kinerja Dosen <span className="text-amber-500">Sistematis & Terpusat.</span></h3>
              <p className="text-foreground/70 text-lg leading-relaxed">
              ...
              <ul className="space-y-4 pt-4">
                <li className="flex items-start gap-3">
                  ...
                  <p className="text-foreground/80"><b>Rekapitulasi Otomatis:</b> Nilai yang disubmit langsung dikalkulasi menjadi skor KPI (Key Performance Indicator) dosen.</p>
                </li>
                <li className="flex items-start gap-3">
                  <Star className="text-amber-500 mt-1 shrink-0 fill-amber-500/20" size={20} />
                  <p className="text-foreground/80"><b>Laporan Pimpinan:</b> Pimpinan (Waket 1 & Ketua) menerima dashboard analisis data matang untuk penjaminan mutu kampus.</p>
                </li>
              </ul>
            </div>
          </div>
      ...
      <footer className="relative mt-10 pt-32 pb-10 overflow-hidden">
        <div className="absolute top-0 left-[-10%] right-[-10%] h-[200px] dark:bg-[#030201] bg-stone-50 rounded-[100%] border-t dark:border-stone-800 border-black/5 shadow-[0_-20px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_-20px_40px_rgba(0,0,0,0.5)] z-0 transition-colors duration-300"></div>
        <div className="relative z-10 max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-bold text-foreground text-lg">STIKOM 22 Januari Kendari</p>
              <p className="text-sm text-foreground/50 mt-1">Sistem Informasi Absensi & Evaluasi Kinerja.</p>
            </div>
          </div>
          <div className="flex items-center gap-4 dark:bg-stone-900/50 bg-black/5 rounded-full p-2 border dark:border-stone-800 border-black/5 transition-colors">
            <span className="px-4 text-xs font-bold text-foreground/50 uppercase tracking-widest">A Project By</span>
